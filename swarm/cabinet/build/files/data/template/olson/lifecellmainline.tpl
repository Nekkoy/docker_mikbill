    ...
    <div class="container">
        ...
        <div class="mainy">
            ...
            <div class="awidget col-md-12">
                ...
                <?php if( empty($this->val["packets"]) ) { ?>
                    <div class="awidget-head">
    
                    </div>
                    <div class="awidget-body no-table-bor">
                        <div class="alert alert-info">
                            <?php echo T_("Тарифная сетка не доступна"); ?>
                        </div>
                    </div>
                <?php } else { ?>
                    <div class="awidget-head">
                        <h3><?php echo T_("Тарифная сетка"); ?></h3>
                    </div>
    
                    <div class="awidget-body no-table-bor">
                        <div class="row row-flex">
                            
                            <?php foreach($this->val["packets"] as $packet){ ?>
                                <!-- <?php var_dump($packet); ?> -->
                                    ...
                                    <div class="packetsRow">
                                        <ul class="price nav nav-pills nav-stacked col-md-12">
                                            <li class="packetHeader">
                                                <?php echo T_($packet['name']); ?>
                                            </li>
        
                                            <li class="packetPrice">
                                                <div>
                                                    <?php echo $packet['price']; ?> <?php echo $this->val['currency']; ?> / 30 днiв
                                                </div>
                                            </li>
        
                                            <li class="packetDescription">
                                                <?php foreach(explode("<br>", $packet['description']) as $line) { ?>
                                            ...
                                            <?php if(!empty($packet['disabled'])) { ?>
                                                <li class="">
                                                    <span class="label label-warning">
                                                        <?php echo T_("Буде доступно з"); ?> <strong><?php echo $packet['blockEnd'] ?></strong>
                                                    </span>
                                                </li>
                                            <?php } ?>
                                        </ul>
                                        ...
                                        <div class="packetFooter">
                                            <form method="post" action="/index/main/lkview/lifecellconfirm">
                                                <input type="hidden" name="gid" value="<?php echo $packet['gid']; ?>"/>
        
                                                <?php if($packet['connected'] == 1) { ?>
                                                    <button type="submit" class="btn btn-warning" <?php echo $packet['disabled'] ?>>
                                                        <?php echo T_("Активировать повторно"); ?>
                                                    </button>
                                                <?php } else { ?>
                                                    <button type="submit" class="button btn" <?php echo $packet['disabled'] ?>>
                                                        <?php echo T_("Подключить"); ?>
                                                    </button>
                                                <?php } ?>
                                            </form>
                                        </div>
