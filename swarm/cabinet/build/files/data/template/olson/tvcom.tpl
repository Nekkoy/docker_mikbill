    ...
    <div class="container">
        ...
        <div class="mainy">
            ...
            <div class="page-title">
                <h2><i class="icon-desktop color"></i> <?php echo T_("TVCom"); ?> </h2>
                <hr/>
            </div>
            <!-- Page title -->
            <?php if( $this->val["subscriptions"]["error"]["number"] == -1 OR $this->val["subscriptions"]["error"]["number"] > 0 ){ ?>
                <div class="alert alert-warning"><?php echo T_($this->val["subscriptions"]["error"]["text"]); ?></div>
            <?php } elseif ( $this->val["subscriptions"]["error"]["number"] == -2 ) { ?>
                <div class="awidget" id="step_0">
                    <div class="awidget-head">
                        <h3><?php echo T_("Учетная запись TVCom"); ?></h3>
                    </div>
    
                    <div class="awidget-body no-table-bor">
                        <div class="alert alert-info">
                            <?php echo T_($this->val["subscriptions"]["error"]["text"]); ?>
                        </div>
                        <form class="form-horizontal" id="form_code_add" method="post" action="/index/main/lkview/tvcom">
                            <div class="form-group">
                                <input type="hidden" class="form-control" id="register" name="register" value="1" required>
                                <div class="col-sm-4">
                                    <button type="submit" id="codeaddb" class="btn btn-info"><?php echo T_("Создать аккаунт"); ?></button>
                                </div>
                            </div>
                        </form>
                    </div>
